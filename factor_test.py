...
def build_factors(df):
    ...
    # 选择20日作为基准（可调整为20/120日）
    factor_df["MA20"] = (
        factor_df.groupby("股票代码")["收盘价"]
        .rolling(window=20, min_periods=20)
        .mean()
        .reset_index(0, drop=True)
    )
    factor_df["BIAS20"] = (
        (factor_df["收盘价"] - factor_df["MA20"]) / factor_df["MA20"] * 100
    )

    # 2.1.2 动量反转因子（MOM）：过去20日收益率（动量）/ 过去5日收益率（反转）
    # 动量因子：过去20日累计收益率（不含当日）
    factor_df["MOM20"] = (
        factor_df.groupby("股票代码")["收盘价"].pct_change(20).reset_index(0, drop=True)
        * 100
    )
    ...
    if "净利润同比增速" in factor_df.columns:
        ...
        factor_df["PROFIT_GROWTH"] = (
            factor_df.groupby("股票代码")["归母净利润"]
            .pct_change()
            .reset_index(0, drop=True)
            * 100
        )
    ...
    print(
        ...
        [
            ...
            if col in ["BIAS20", "MOM20", "REV5", "ROE", "PROFIT_GROWTH"]
        ],
    )
    factor_df.to_csv("final_data/沪深300_3年简单因子分析.csv", index=False)
    return factor_df
...
def preprocess_factors(factor_df):
    ...
    # 定义需要处理的因子列表
    factor_list = ["BIAS20", "MOM20", "REV5", "ROE", "PROFIT_GROWTH"]
...
def factor_analysis(processed_df, factor_list):
    ...
    for factor in factor_list:
        # 计算下期收益（未来1日涨跌幅）
        processed_df["next_return"] = processed_df.groupby("股票代码")["涨跌幅"].shift(
            -1
        )

        # 按日期计算秩相关系数（Spearman）
        daily_ic = (
            processed_df.groupby("日期")
            .apply(
                lambda x: x[[factor, "next_return"]]
                .corr(method="spearman")
                .loc[factor, "next_return"]
            )
            .dropna()
        )
...
def stratified_backtest(processed_df, factor_list):
    ...
    for factor in factor_list:
        ...
        processed_df["factor_rank"] = processed_df.groupby("日期")[factor].rank(
            method="min", pct=True
        )
        ...
        layer_returns = (
            processed_df.groupby(["日期", "layer"])["next_return"].mean().reset_index()
        )

        # 5.3 计算累计收益
        layer_cum_return = (
            layer_returns.pivot_table(
                index="日期", columns="layer", values="next_return"
            )
            .fillna(0)
            .cumsum()
        )
        ...
        for layer in ["Layer1", "Layer2", "Layer3", "Layer4", "Layer5"]:
            if layer in layer_cum_return.columns:
                ...
                # 日均收益
                daily_return = layer_returns[layer_returns["layer"] == layer][
                    "next_return"
                ].mean()
                # 胜率（正收益天数占比）
                win_rate = (
                    layer_returns[layer_returns["layer"] == layer]["next_return"] > 0
                ).sum() / len(layer_returns[layer_returns["layer"] == layer])
                # 最大回撤（简化计算）
        ...
        if (
            "Layer5" in layer_cum_return.columns
            and "Layer1" in layer_cum_return.columns
        ):
            layer_cum_return["LongShort"] = (
                layer_cum_return["Layer5"] - layer_cum_return["Layer1"]
            )
        ...
        layer_cum_return[["Layer1", "Layer2", "Layer3", "Layer4", "Layer5"]].plot(
            ax=plt.gca()
        )
        if "LongShort" in layer_cum_return.columns:
            plt.plot(
                layer_cum_return.index,
                layer_cum_return["LongShort"],
                label="多空组合",
                linestyle="--",
                linewidth=2,
        ...
        plt.savefig(
            f"final_data/{factor}_分层收益曲线.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def generate_report(analysis_results, backtest_results, factor_list):
    ...
    report_content.append("- 测试标的：沪深300成分股（样本量：150只）")
    report_content.append("- 测试时间：2024-01-01 至 2026-03-19")
    report_content.append(
        "- 测试因子：量价类（BIAS60、MOM20、REV5）、财务类（ROE、PROFIT_GROWTH）"
    )
    ...
    if effective_factors:
        report_content.append(
            f"- 有效因子：{', '.join(effective_factors)}（IC绝对值>0.02）"
        )
    else:
        report_content.append(
            "- 暂无显著有效因子（IC绝对值均<0.02），建议调整因子参数或扩大样本"
        )
